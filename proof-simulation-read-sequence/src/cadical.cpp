...
void App::print_usage (bool all) {
  ...
  printf  ("  -core   <core lemma file>     Trimmed proof by DRAT that contains only core lemmas (NOT needed by simulating solver)\n"
           "  -sequence <sequence file>     Branching sequence file to be read by simulating solver\n\n"
...
int App::main (int argc, char **argv) {
  ...
  const char *branching_sequence_path = 0;
  bool read_sequence = true;       // --rs=1  (default)
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp (argv[i], "-h") || !strcmp (argv[i], "--help") ||
      ...
      status = true;
             ...
             !strcmp (argv[i], "--rs=1"))
      read_sequence = true;   // default
    else if (!strcmp (argv[i], "--rs=false") ||
             !strcmp (argv[i], "--rs=0"))
      read_sequence = false; 
  ...
  solver->internal->read_sequence = read_sequence;         // default true (--rs=1). When --rs=0, this system works as the original Cadical solver.
