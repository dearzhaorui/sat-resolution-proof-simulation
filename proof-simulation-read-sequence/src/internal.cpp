...
namespace CaDiCaL {
  ...
  void Internal::finish_added_clause_with_id (uint64_t id, bool restore) {
    ...
    cout << "restore finish_added_clause_with_id...... " << endl;
    add_new_original_clause (id);
    original.clear ();
  }
  ...
  int Internal::cdcl_loop_with_inprocessing () { // printing branching_sequence
    cout << "read_sequence = " << read_sequence << endl;
    
    int res = 0;
    
    // default true (--rs=1)
    if(read_sequence) {
      cout << "This system will perform proof simulation procedure that reads a branching sequence." << endl;
      res = read_branch_sequence();
    }
    // Cadical loop, generating proofs for unsatisfiable instances.
    else {
      cout << "This system will work as an original Cadical solver." << endl;
      res = cdcl_loop_with_inprocessing2(); 
    }
    
    //res = print_core_lemmas(); // just print the clean core lemmas
  ...
  int Internal::cdcl_loop_with_inprocessing2 () {  // original Cadical main loop
  ...
  res = decide (); // next decision
    ...
    cout << "stats.numAnalyze " << stats.numAnalyze << endl;
