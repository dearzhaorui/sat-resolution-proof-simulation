...
inline void Internal::analyze_literal (int lit, int &open,
  ...
  if (v.level < level) {
    ...
    //cout << " seenl " << lit << ",lv" << var(lit).level<< "t" << var(lit).trail << " ";
  }
...
//inline int Internal::otfs_find_backtrack_level (int &forced) {
  ...
  //const int other = lits[0];
  //lits[0] = forced;
  //lits[pos] = other;
  
  ////cout << "other.l " << var(other).level << ", uip.l " << var(forced).level << ", level " << level << ", pos " << pos << endl;
  
  //if (pos > 1) {
    //++stats.otfsPos2;
    //remove_watch (watches (other), conflict);
    //update_watch_size (watches (lits[1]), lits[0], conflict);
    //watch_literal (forced, lits[1], conflict);
  //}
  //else ++stats.otfsPos1;
...
void Internal::clear_flags_and_count_opens (Clause* c) {
  clear_analyzed_literals ();  // must set all flag.seen to false, because bump_variables may also set it to true
  ...
  for (auto const& lit : *c) {
    ...
    f.seen = true;
    analyzed.push_back (lit);
    if (!l.seen.count++) {   // the open is equal to this seen.count
      levels.push_back (v.level);
    }
  }
}
  ...
  for (;;) {
    ...
    if (otfs && resolved > 0 && antecedent_size > 2 &&
      ...
      if (open == 1) {
        ...
        conflict = 0;
        ++stats.analysis;
        stats.negUipIsLastDecs += (-forced == lastDec);
        // Clean up.
        //
