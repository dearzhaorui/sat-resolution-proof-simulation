...
Clause *Internal::new_clause2 (bool red, int glue) {
  assert (clause.size () <= (size_t) INT_MAX);
  const int size = (int) clause.size ();
  assert (size >= 2);
...
void Internal::assign_original_unit (uint64_t id, int lit) {
  ...
  // Proof simulation: the following two lines of code are replaced by the rest code below
  //if (propagate ())
    //return;
  int propagated_orig = propagated;
  if (propagate ()) {
    [[maybe_unused]] uint ts = trail.size(); 
    propagated = propagated_orig;
    propagate2 ();  // also propagate the copied DB
    assert(!conflict);
...
void Internal::add_new_original_clause (uint64_t id) {
  ...
  if (skip) {
    ...
    if (!size) {
      ...
      Clause *c2 = new_clause2 (false, glue);
      c->id = new_id;
      clause_id--;
      watch_clause (c);
      watch_clause2 (c2);
      clause_map[c] = c2;
      clause_map_to_orig_cls[c2] = c;
...
Clause *Internal::new_learned_redundant_clause (int glue) {
  ...
  Clause *res2 = new_clause2 (true, glue); // Proof simulation: make a copy of each new clause
  clause_map[res] = res2;  // Proof simulation: Mapping original res --> res2
  clause_map_to_orig_cls[res2] = res;  // Proof simulation: Mapping the copy res2 --> res, used only in eagerly_subsume_recently_learned_clauses() in analyze for mark_garbage()
  if (proof) {
    proof->add_derived_clause (res, lrat_chain);
    proof->add_derived_clause (res2, lrat_chain);
  }
    
  assert (watching ());
  watch_clause (res);
  watch_clause2 (res2); // Proof simulation: the second watchlist table
  res2->used = 1 + (glue <= opts.reducetier2glue);
