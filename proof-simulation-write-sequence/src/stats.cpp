...
void Stats::print (Internal *internal) {
  ...
  if (all || stats.decisions) {
    PRT ("decisionsSimRead: %15" PRId64 "   %10.2f    per second",
         stats.decisionsSimRead, relative (stats.decisionsSimRead, t));
  ...
  std::cout << "c #analysis " << stats.analysis << " ,negUipIsLastDecs " << stats.negUipIsLastDecs << " ( " << (double)stats.negUipIsLastDecs/stats.analysis*100 << "% )" << std::endl;
