...
void App::print_usage (bool all) {
  ...
  printf  ("  -core   <core lemma file>     Trimmed proof by DRAT that contains only core lemmas\n"
           "  -sequence <sequence file>     Branching sequence file to be wroten by the modified solver when constructing the sequence\n\n"
...
int App::main (int argc, char **argv) {
  ...
  const char *core_lemmas_path = 0, *branching_sequence_path = 0;
  bool write_sequence = true;     // --ws=1
  bool read_sequence = false;    // --rs=0
  const char *simulation_lemma_path = 0;
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp (argv[i], "-h") || !strcmp (argv[i], "--help") ||
    ...
    else if (!strcmp (argv[i], "-core")) { // core_lemmas_path
      if (++i == argc)
        APPERR ("argument to '-core' missing");
      else if (core_lemmas_path)
        APPERR ("multiple core lemmas file options '-core %s' and '-core %s'",
                core_lemmas_path, argv[i]);
      else if (!force_writing && most_likely_existing_cnf_file (argv[i]))
        APPERR ("core lemmas file '%s' most likely existing CNF (use '-f')",
                argv[i]);
      else
        core_lemmas_path = argv[i];
    } else if (!strcmp (argv[i], "-sequence")) { // branching_sequence_path
      if (++i == argc)
        APPERR ("argument to '-sequence' missing");
      else if (branching_sequence_path)
        APPERR ("multiple proof simulation output file options '-sequence %s' and '-sequence %s'",
                branching_sequence_path, argv[i]);
      else if (!force_writing && most_likely_existing_cnf_file (argv[i]))
        APPERR ("proof simulation output file '%s' most likely existing CNF (use '-f')",
                argv[i]);
      else if (!File::writable (argv[i]))
        APPERR ("proof simulation output file '%s' not writable", argv[i]);
      else
        branching_sequence_path = argv[i];
    } 
    else if (!strcmp (argv[i], "-simlem")) { // simulation_lemma_path
      if (++i == argc)
        APPERR ("argument to '-simlem' missing");
      else if (simulation_lemma_path)
        APPERR ("multiple simu lemmas file options '-simlem %s' and '-simlem %s'",
                simulation_lemma_path, argv[i]);
      else if (!force_writing && most_likely_existing_cnf_file (argv[i]))
        APPERR ("simu lemmas file '%s' most likely existing CNF (use '-f')",
                argv[i]);
      else
        simulation_lemma_path = argv[i];
    } 
      ...
      status = true;
             ...
             !strcmp (argv[i], "--ws=true") ||
             !strcmp (argv[i], "--ws=1"))
      write_sequence = true;  // default
    else if (!strcmp (argv[i], "--rs") ||
             !strcmp (argv[i], "--rs=true") ||
             !strcmp (argv[i], "--rs=1"))
      read_sequence = true;   // default false
  ...
  string dimacsPath = dimacs_path;
  char * dp = new char[dimacsPath.length() + 1];
  strcpy(dp, dimacsPath.c_str());
  string delim = "/";
  ...
  int size = v_strs.size();
  string cnf_name = v_strs[size-1];
  solver->internal->cnf_name = cnf_name;
  
  if (core_lemmas_path) {
    string core_lemma(core_lemmas_path);
    solver->internal->core_lemmas_path = core_lemma;
  }
  if (branching_sequence_path) {
    string out(branching_sequence_path);
    solver->internal->branching_sequence_path = out;
  }
  if (simulation_lemma_path) {
    string sim(simulation_lemma_path);
    solver->internal->simulation_lemma_path = sim;
  }
  
  solver->internal->write_sequence = write_sequence;       // default true, --ws=1
  solver->internal->read_sequence = read_sequence;         // default false, --rs=0
