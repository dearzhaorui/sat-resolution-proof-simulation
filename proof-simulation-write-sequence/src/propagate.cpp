...
bool Internal::propagate () {
  assert(!conflict);
  assert(clauses.size()%2 == 0);  // Proof simulation: original DB + the deplicated DB
  ...
  while (!conflict && propagated != trail.size ()) {
    ...
    while (i != eow) {
      ...
      if (w.binary ()) {
        ...
        if (w.clause->garbage) {
          assert(clause_map[w.clause]->garbage);
          j--;
          continue;
        }
  ...
  if (searching_lucky_phases) {
    ...
    if (!conflict)
      ...
      if (stable)
        stats.stabconflicts++;
      stats.conflicts++;
      lastDec = (level == 0? 0 : control.back().decision);  // Proof simulation: for statistics
...
// Proof simulation: this method is used to propagate only the deplicated DB 
// by visiting the deplicated watchlists in order to not change the literals' order in original clauses
// and the original watchlists
bool Internal::propagate2 () {
  assert(!conflict);
  assert(clauses.size()%2 == 0);  // Proof simulation: original DB + the deplicated DB
  ...
  while (!conflict && propagated != trail.size ()) {
    ...
    while (i != eow) {
      ...
      if (w.binary ()) {
        ...
        if (w.clause->garbage) {
          assert(clause_map_to_orig_cls[w.clause]->garbage);
          j--;
          continue;
        }
            ...
            *k = lit;
            ...
            if (opts.chrono > 1) {
              ...
              if (other_level > var (lit).level) {
                ...
                lits[0] = other;
                lits[1] = s;
                watch_literal2 (s, other, w.clause);
