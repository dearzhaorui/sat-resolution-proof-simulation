...
struct Internal {
  ...
  vector<signed char> possi_empows;    // signed marks [1,max_var]
  ...
  vector<Watches> wtab2;        // table of watches for all literals in the duplicated DB
  ...
  // Proof simulation: new variables
  vector<int> lemma, other;
  string cnf_name;
  ofstream out; 
  string core_lemmas_path;
  string branching_sequence_path; // branching sequence
  string simulation_lemma_path;
  bool write_sequence;
  bool read_sequence;
  unordered_map<Clause*, Clause*> clause_map; // mapping from original clause to its duplicated clause
  unordered_map<Clause*, Clause*> clause_map_to_orig_cls; // mapping from the duplicated clause to its original clause, used only in eagerly_subsume_recently_learned_clauses() in analyze for mark_garbage()
  uint cc;
  int numCoreLemmas = 0;
  //--------------------------------
  ...
  bool watching () const { return !wtab.empty () && !wtab2.empty (); }
  ...
  Watches &watches2 (int lit) { return wtab2[vlit (lit)]; }
  ...
  bool possi_empow_lit (int lit) const {
    signed char res = possi_empows[vidx (lit)];
    if (lit < 0)
      res = -res;
    return res > 0;
  }
  void mark_possi_empow_lit (int lit) {
    assert (!possi_empow_lit (lit));
    possi_empows[vidx (lit)] = sign (lit);
    assert (possi_empow_lit (lit));
    assert (!possi_empow_lit (-lit));
  }
  void unmark_possi_empow_lit (int lit) {
    assert (possi_empow_lit (lit));
    possi_empows[vidx (lit)] = 0;
    assert (!possi_empow_lit (lit));
    assert (!possi_empow_lit (-lit));
  }
  ...
  inline void watch_literal2 (int lit, int blit, Clause *c) {
    assert (lit != blit);
    Watches &ws = watches2 (lit);
    ws.push_back (Watch (blit, c));
    LOG (c, "watch %d blit %d in", lit, blit);
  }
  ...
  inline void watch_clause2 (Clause *c) {
    const int l0 = c->literals[0];
    const int l1 = c->literals[1];
    watch_literal2 (l0, l1, c);
    watch_literal2 (l1, l0, c);
  }
  ...
  Clause *new_clause2 (bool red, int glue = 0);
  ...
  Clause *on_the_fly_strengthen2 (Clause *conflict, int lit);
  Clause *on_the_fly_strengthen3 (Clause *conflict, int lit);
  void analyze ();
  ...
  int cdcl_loop_with_inprocessing1 ();
  int cdcl_loop_with_inprocessing2 ();
  void reset_solving ();
  int solve (bool preprocess_only = false);
  void finalize (int);
  
  // Proof simulation: new methods
  bool has_next_lemma (ifstream& in);
  ...
  bool isUnit (int lit) const {
    assert (-max_var <= lit);
    assert (lit);
    assert (lit <= max_var);
    return (vals[lit] != 0 and vtab[vidx (lit)].level == 0);
  }
