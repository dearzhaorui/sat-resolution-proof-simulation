...
namespace CaDiCaL {
  ...
  Internal::Internal ()
    : mode (SEARCH), unsat (false), iterating (false),
      ...
      write_sequence (true), read_sequence (false),
      opts (this),
#ifndef QUIET
  ...
  void Internal::enlarge (int new_max_var) {
    ...
    enlarge_only (wtab2, 2 * new_vsize);
    ...
    enlarge_zero (possi_empows, new_vsize);
    vsize = new_vsize;
  }
  ...
  int Internal::cdcl_loop_with_inprocessing () { // printing branching_sequence
    if (read_sequence) write_sequence = false;
    assert(write_sequence);
    cout << "write_sequence " << write_sequence << endl;
    
    if (!write_sequence) {
      cout << "This version only support writing the sequence! Use flag ' --ws=0 '!" << endl;
      exit(0);
    }
