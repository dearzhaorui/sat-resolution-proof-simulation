...
//inline int Internal::find_conflict_level (int &forced) {
  ...
  ////------- do the same for copied clause
  //Clause* conflict2 = clause_map[conflict];
  //int *lits2 = conflict2->literals;
  //assert(size == conflict2->size);
  
  //for (int i = 0; i < 2; i++) {

    //const int lit = lits2[i];
    ...
    //if (highest_position > 1) {
      //LOG (conflict, "unwatch %d in", lit);
      //remove_watch (watches2 (lit), conflict2);
    //}

    //lits2[highest_position] = lit;
    //lits2[i] = highest_literal;

    //if (highest_position > 1) {
      //watch_literal2 (highest_literal, lits2[!i], conflict2);
    //}
  //}
...
void Internal::eagerly_subsume_recently_learned_clauses (Clause *c) {
  ...
  mark (c);
  int64_t lim = stats.eagertried + opts.eagersubsumelim*2; // Proof simulation: *2 because there are copies of clauses
  ...
  while (it != begin && stats.eagertried++ <= lim) {
    ...
    if (c == d)
      continue;
    if (d == clause_map[c]) { // Proof simulation: skip comparing original clause c with its own duplication d
      continue;
    }
    if (d->garbage)
      continue;
    ...
    // Proof simulation: here d should be the duplicated clause, because DB is traversed from end to front.
    // when d is subsumed, the original clause is also subsumed, so mark it as garbage
    mark_garbage (clause_map_to_orig_cls[d]);
  }
  unmark (c);
#ifdef LOGGING
...
Clause *Internal::on_the_fly_strengthen2 (Clause *new_conflict, int uip) {
  assert (new_conflict);
  assert (new_conflict->size > 2);
  LOG (new_conflict, "applying OTFS on lit %d", uip);
  auto sorted = std::vector<int> ();
  auto sorted2 = std::vector<int> ();
  sorted.reserve (new_conflict->size);
  sorted2.reserve (new_conflict->size);
  assert (sorted.empty ());
  assert (sorted2.empty ());
  ++stats.otfs.strengthened;

  int *lits = new_conflict->literals;
  
  Clause* new_conflict2 = clause_map[new_conflict];
  int *lits2 = new_conflict2->literals;
  assert(new_conflict->size == new_conflict2->size);
  
  assert (lits[0] == uip || lits[1] == uip);
  
  int i = 0;
  for( ; i < new_conflict2->size; ++i) {
    if (lits2[i] == uip) break;
  }
  assert(i < new_conflict2->size);
  if(i >= 2) { // move uip to pos 0
    remove_watch (watches2 (lits2[0]), new_conflict2);
    watch_literal2 (uip, lits2[1], new_conflict2);
    lits2[i] = lits2[0];
    lits2[0] = uip;
  }
  
  assert (lits2[0] == uip || lits2[1] == uip);
  
  const int other_init = lits[0] ^ lits[1] ^ uip;
  const int other_init2 = lits2[0] ^ lits2[1] ^ uip;
  ...
  int new_size2 = 0;
  for (int i = 0; i < old_size; ++i) {
    const int other = lits2[i];
    sorted2.push_back (other);
    if (var (other).level) {
      lits2[new_size2++] = other;
    }
  }
  assert(new_size == new_size2);
  ...
  const int other2 = lits2[0] ^ lits2[1] ^ uip;
  lits2[0] = other2;
  lits2[1] = lits2[--new_size2];
  ...
  if (other_init2 != other2) {
    remove_watch (watches2 (other_init2), new_conflict2);
  }
  remove_watch (watches (uip), new_conflict);
  remove_watch (watches2 (uip), new_conflict2);
  ...
  // sort the clause
  {
    ...
    int highest_pos2 = 0;
    int highest_level2 = 0;
    for (int i = 1; i < new_size2; i++) {
      const unsigned other = lits2[i];
      assert (val (other) < 0);
      const int level = var (other).level;
      assert (level);
      LOG ("checking %d", other);
      if (level <= highest_level2)
        continue;
      highest_pos2 = i;
      highest_level2 = level;
    }
    LOG ("highest lit is %d", lits2[highest_pos2]);
    if (highest_pos2 != 1) {
      swap (lits2[1], lits2[highest_pos2]);
    }
    LOG ("removing %d literals", new_conflict2->size - new_size2);
    otfs_strengthen_clause (new_conflict2, uip, new_size2, sorted2);
    assert (new_size2 == new_conflict2->size);

    assert(highest_level == highest_level2);
  }
  ...
  if (other_init2 != other2) {
    watch_literal2 (other2, lits2[1], new_conflict2);
  }
  else {
    update_watch_size (watches2 (other2), lits2[1], new_conflict2);
  }
  watch_literal (lits[1], other, new_conflict);
  watch_literal2 (lits2[1], other2, new_conflict2);
...
Clause *Internal::on_the_fly_strengthen3 (Clause *new_conflict, int uip) {
  assert (new_conflict);
  assert (new_conflict->size > 2);
  LOG (new_conflict, "applying OTFS on lit %d", uip);
  auto sorted = std::vector<int> ();
  ...
  const int other_init = lits[0] ^ lits[1] ^ uip;
   int other_init2 = lits2[0] ^ lits2[1] ^ uip;
  
  if (other_init2 != other_init) {
    int idx_other_init2 = (lits2[0] == uip? 1 : 0);
    int i = 2;
    for( ; i < new_conflict2->size; ++i) {
      if (lits2[i] == other_init) break;
    }
    assert(i < new_conflict2->size);
    
    lits2[idx_other_init2] = other_init; 
    lits2[i]               = other_init2; 
    remove_watch (watches2 (other_init2), new_conflict2);
    watch_literal2 (other_init, uip, new_conflict2);
    update_watch_size (watches2 (uip), other_init, new_conflict2);
    other_init2 = other_init;
  }
...
inline void Internal::otfs_subsume_clause (Clause *subsuming,
  ...
  if (subsumed->redundant || !subsuming->redundant) {
    ...
    mark_garbage (clause_map[subsumed]);
    return;
  }
  LOG ("turning redundant subsuming clause into irredundant clause");
  subsuming->redundant = false;
  clause_map[subsuming]->redundant = false;
  if (proof)
...
void Internal::analyze () {
  ...
  for (;;) {
    ...
    if (otfs && resolved > 0 && antecedent_size > 2 &&
      ...
      reason = on_the_fly_strengthen2 (reason, uip);  // Proof simulation: updated by considering the duplicated DB
      ...
      if (open == 1) {
        ...
        conflict = 0;
        ++stats.analysis;
        stats.negUipIsLastDecs += (-forced == lastDec); // Proof simulation: statistics
        // Clean up.
        //
  ...
  if (uip) {
    ...
    ++stats.analysis;
    stats.negUipIsLastDecs += (uip == lastDec);
  } else
    learn_empty_clause ();
